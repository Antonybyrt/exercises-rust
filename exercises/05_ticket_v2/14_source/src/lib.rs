...
pub enum TicketNewError {
    ...
    InvalidStatus(String, #[source] ParseStatusError),
}
...
impl Ticket {
    pub fn new(title: String, description: String, status: String) -> Result<Self, TicketNewError> {
        ...
        // Conversion du statut en énuméré `Status`
        let status_clone = status.clone();  // Clone pour utiliser dans l'erreur
        let status = Status::try_from(status).map_err(|e| {
            TicketNewError::InvalidStatus(status_clone, e)
        })?;
